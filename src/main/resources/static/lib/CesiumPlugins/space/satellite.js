/*
 * satellite-js v1.1 
 * 来源 https://github.com/shashwatak/satellite-js
 * 修改 http://www.dehui.info
 */
var satellite = (function () {
    ...
    function topocentric(observer_coords, satellite_coords) {
        ...
        var observer_ecf = geodetic_to_ecf(observer_coords);       //把测站的地理坐标转换成ECEF

        var rx = satellite_coords["x"] - observer_ecf["x"];            //把目标的ECEF转换到测站系
    ...
    *   | R * sin(E)               |
    ******************************************************/
    satellite.rae_station = function (rae) {
        'use strict';
        var x = rae["range_sat"] * Math.cos(rae["elevation"]) * Math.cos(rae["azimuth"]);
        var y = rae["range_sat"] * Math.cos(rae["elevation"]) * Math.sin(rae["azimuth"]);
        var z = rae["range_sat"] * Math.sin(rae["elevation"]);
    ...
    /* 测站系xyz --> ECEF */
    satellite.station_ecef = function (observer_coords, pos) {
        //输入：测站地理坐标（）、测站系位置（X,Y,Z）

        'use strict';
        var ay = Math.PI / 2 - observer_coords["latitude"];
        var az = Math.PI - observer_coords["longitude"];

        var cy = buildRotationMatrix2(-ay);
        var cz = buildRotationMatrix3(-az);
        var cp = buildRotationMatrix0();

        var pos_station = satellite.geodetic_to_ecf(observer_coords);
        var r0 = [pos_station["x"], pos_station["y"], pos_station["z"]];
        var r = [pos["x"], pos["y"], pos["z"]];

        //cz*cy*cp*r + r0;
        var temp = multiply2dBy2d(cz, cy);
        var temp2 = multiply2dBy2d(temp, cp);
        var temp3 = multiply2dBy1d(temp2, r);
        var temp4 = add1dTo1d(temp3, r0);

        return { x: temp4[0], y: temp4[1], z: temp4[2] };

    }

    /******************************************************
    *   RAE --> ECEF
    *   输入： observer_coords 测站经纬高地理坐标
    *           rae(距离、方位、俯仰）公里、弧度
    ******************************************************/
    satellite.rae_ecef = function (observer_coords, rae) {
        'use strict';
        //var rae = { range_sat: 1891.420816, elevation: -0.30475194069, azimuth: 4.8 };
        var pos_s = satellite.rae_station(rae);
        //var observer_coords = { longitude: 1.90037, latitude: 0.598049, height: 0 };
        var pos_ecef = satellite.station_ecef(observer_coords, pos_s);
    ...
    *   输入： observer_coords 测站经纬高地理坐标
    ******************************************************/
    satellite.rae_eci = function (observer_coords, rae, gmst) {
        'use strict';
        var pos_ecef = satellite.rae_ecef(observer_coords, rae);
        var pos_eci = satellite.ecf_to_eci(pos_ecef, gmst);
    ...
    *   输入：ECI坐标、时间
    ******************************************************/
    satellite.ecef_to_geodetic = function (ecf_coords, gmst) {
        var eci = satellite.ecf_to_eci(ecf_coords, gmst);
        var geo = satellite.eci_to_geodetic(eci, gmst);
        return geo;
    }
    ...
    /*********************************************************
    *   计算目标的位置
    *   输入：flag:坐标系标记 tle1, tle2, ymd
    *   输出： { x: position["x"], y: position["y"], z: position["z"], lon: position_gd["longitude"], lat: position_gd["latitude"], h: position_gd["height"] }
    ********************************************************/
    satellite.calculate_sat_pos_ex = function (flag, tle1, tle2, ymd) {

        var gmst = satellite.gstime_from_date(ymd.year, ymd.month, ymd.day, ymd.hour, ymd.minute, ymd.second);
        var satrec = satellite.twoline2satrec(tle1, tle2);
        var position_and_velocity = satellite.propagate(satrec, ymd.year, ymd.month, ymd.day, ymd.hour, ymd.minute, ymd.second);

        if (position_and_velocity["position"] == null)
            return { x: 0, y: 0, z: 0, lon: 0, lat: 0, h: 0 };

        var position_gd = satellite.eci_to_geodetic(position_and_velocity["position"], gmst);

        var position;
        if (flag == "ECI") {
            position = position_and_velocity["position"];
        }
        else {
            position = satellite.eci_to_ecf(position_and_velocity["position"], gmst);
        }

        return { x: position["x"], y: position["y"], z: position["z"], longitude: position_gd["longitude"], latitude: position_gd["latitude"], height: position_gd["height"] };

    }

    // 获取目标的运行周期
    satellite.get_period = function (tle2) {
        // 输入：TLE的第二行
        // 输出：周期（分钟单位）

        return parseInt(1440.0 / parseFloat(tle2.substring(52, 63)));
...
/***************************************************************
*   以下函数从universe.js中的mathtools中获取的函数 
****************************************************************/
/**
 ...
 */
function buildRotationMatrix1(x) {
...
/**
    ...
    * @returns {Number[][]} rotationMatrix  (3x3)
    */
function buildRotationMatrix2(x) {
...
/**
    ...
    * @returns {Number[][]} rotationMatrix  (3x3)
    */
function buildRotationMatrix3(x) {
...
/**
    ...
    */
function subtract1dMinus1d(x, y) {
    var x1 = x.length,
        y1 = y.length,
        returnVal = [],
        i;
